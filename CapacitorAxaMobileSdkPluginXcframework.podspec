...
Pod::Spec.new do |s|
  ...
  s.dependency 'CAMobileAppAnalytics/xcframework', '<= 24.7.4-beta.1.0'
  s.script_phases = [
      {   :name => 'Precompile',
         :script => 'cd "${PODS_ROOT}"
                    irb <<EOF
...
project.targets.each do |target|
    if target.name == "CapacitorAxaMobileSdkPluginXcframework" && !(target.headers_build_phase.file_display_names.include? "CAMDOReporter.h")
